...
export function f_draw(a_bmd, a_settings) {
	...
	c_svg_g1.setAttributeNS(
		null,
		"transform",
		"translate("
		+ String(-1 * c_left_x16 * c_sz)
		+ ","
		+ String(-1 * c_top_y16 * c_sz)
		+ ") scale(" + String(c_sz)
		+ ","
		+ String(c_sz) + ")"
	);
	...
	for (let i1 = a_settings["min_zoom_level"]; i1 <= a_settings["max_zoom_level"]; i1++) {
		...
		for (let i2 = 0; i2 < a_bmd["ur_route_child_shape_segment_arrays"].length; i2++) {
			...
			for (let i3 = 0; i3 < c_polyline.length; i3++) {
				c_polyline[i3]["lon"] = f_lonlat_xy(
					c_polyline[i3]["x"],
					"x_to_lon",
					c_zoom_level
				);
				c_polyline[i3]["lat"] = f_lonlat_xy(
					c_polyline[i3]["y"],
					"y_to_lat",
					c_zoom_level
				);
			...
			for (let i3 = 0; i3 < c_polyline.length; i3++) {
				...
				for (let i4 = 0; i4 < c_polyline[i3]["ids"].length; i4++) {
					const c_near_stops = a_bmd["shape_points"][
						c_polyline[i3]["ids"][i4]
					]["near_stops"];
			...
			for (let i5 = 0; i5 < a_bmd["parent_routes"].length; i5++) {
				if (c_parent_route_id === a_bmd["parent_routes"][i5]["parent_route_id"]) {
					if (
						a_bmd["parent_routes"][i5]["route_color"] === undefined
						|| a_bmd["parent_routes"][i5]["route_color"] === ""
					) {
						//l_route_color = "808080"
						a_bmd["parent_routes"][i5]["route_color"] = Math.round(
							(Math.random() * 15)).toString(16) + "F"
							+ Math.round((Math.random() * 15)).toString(16) + "F"
							+ Math.round((Math.random() * 15)).toString(16) + "F"; //本来はFFFFFF
					}
				...
				for (let i3 in c_cut_polyline["curves"]) {
					if (a_settings["leaflet"] === true) {
						...
						for (let i4 = 0; i4 < c_cut_polyline["curves"][i3].length; i4++) {
							...
							const c_curve = L.curve(
								c_cut_polyline["curves"][i3][i4]["curve"], {
								"color": "#" + l_route_color/*a_bmd["ur_routes"][i2]["route_color"]*/,
								"weight": c_cut_polyline["curves"][i3][i4]["width"] * 256 / c_zoom_ratio
							});
						...
						for (let i4 = 0; i4 < c_cut_polyline["curves"][i3].length; i4++) {
							...
							c_svg_path.setAttributeNS(null,
								"fill",
								"none");
							c_svg_path.setAttributeNS(null,
								"stroke",
							...
							c_svg_path.setAttributeNS(null,
								"stroke-width",
								c_cut_polyline["svg_paths"][i3][i4]["stroke-width"]);
			...
			for (let i3 = 0; i3 < c_cut_polyline["stop_array"].length; i3++) {
				if (a_settings["leaflet"] === true) {
					a_bmd["layer_zoom_" + String(i1)].addLayer(
						L.circle(c_cut_polyline["stop_array"][i3], {
							"radius": 2,
							"stroke": 1,
							"color": "#000000",
							"fill": true,
		...
		if (a_settings["leaflet"] !== true) {
			for (let i2 = 0; i2 < a_bmd["parent_stations"].length; i2++) {
				...
				c_svg_text.setAttributeNS(null,
					"x",
					f_lonlat_xy(
						a_bmd["parent_stations"][i2]["stop_lon"],
						"lon_to_x", 16
					) + 8 * (2 ** (-8))
				);
				c_svg_text.setAttributeNS(null,
					"y",
					f_lonlat_xy(
						a_bmd["parent_stations"][i2]["stop_lat"],
						"lat_to_y",
						16
					) + 16 * (2 ** (-8))
				);
	...
	if (a_settings["leaflet"] === true) {
		...
		for (let i1 = 0; i1 < a_bmd["parent_stations"].length; i1++) {
			L.marker({
				"lon": a_bmd["parent_stations"][i1]["stop_lon"],
				"lat": a_bmd["parent_stations"][i1]["stop_lat"]
			}, {
				"icon": L.divIcon({
					"html": "<span style=\"writing-mode: "
						+ a_settings["writing_mode"]
						+ ";\" onclick=\"f_set_stop_id('"
						+ a_bmd["parent_stations"][i1]["stop_id"]
						+ "');\">"
						+ a_bmd["parent_stations"][i1]["stop_name"]
						+ "</span>",
					className: "className",
					iconSize: [256, 256],
					iconAnchor: [-4, -4]
	...
	function f_show_search_route(a_start_parent_station, a_end_parent_station) {
		const c_route_se = f_search_route(
			a_start_parent_station,
			a_end_parent_station,
			a_bmd,
			c_parent_station_index
		);
		//parent_routeでまとめる
		...
		for (let i1 = 0; i1 < a_bmd["parent_routes"].length; i1++) {
			...
			for (let i2 in c_groups["parent_route_id_" + c_parent_route_id]) {
				...
				c_groups["parent_route_id_" + c_parent_route_id][i2].setStyle({
					"color": l_color
				});
			}
		}
