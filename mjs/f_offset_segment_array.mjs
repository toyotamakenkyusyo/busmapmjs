			...
			for (let i2 = 0; i2 < l_segment_array.length; i2++) {
				...
				if (
					...
					&& (
						...
						&& (
							(l_segment_array[i2 - 1]["st"] < l_segment_array[i2 - 1]["et"])
							|| (l_segment_array[i2 + 1]["st"] < l_segment_array[i2 + 1]["et"])
						)
					) //順序が逆（隣の少なくとも一方は順）
					&& (
						(a_settings["separate_stops"] === true
							&& l_segment_array[i2]["s_stop"] === false
							&& l_segment_array[i2]["e_stop"] === false
						) //停車地を分ける、停車地がない場合
...
function f_offset_segment_array_once(a_segment_array, a_settings) {
	...
	const c_vnx = a_segment_array[a_segment_array.length - 1]["ex"]
		- a_segment_array[a_segment_array.length - 1]["sx"];
	const c_vny = a_segment_array[a_segment_array.length - 1]["ey"]
		- a_segment_array[a_segment_array.length - 1]["sy"];
	const c_vnn = ((c_vnx * c_vnx) + (c_vny * c_vny)) ** 0.5;
	a_segment_array[a_segment_array.length - 1]["exy"] = [{}];
	a_segment_array[a_segment_array.length - 1]["exy"][0]["x"] = c_vnz * c_vny / c_vnn
		+ a_segment_array[a_segment_array.length - 1]["ex"];
	a_segment_array[a_segment_array.length - 1]["exy"][0]["y"] = (-1) * c_vnz * c_vnx / c_vnn
		+ a_segment_array[a_segment_array.length - 1]["ey"];
	//途中
	for (let i1 = 0; i1 < a_segment_array.length - 1; i1++) {
		if (a_segment_array[i1 + 1]["delated"] !== false) {
			f_offset_point(
				a_segment_array[i1],
				a_segment_array[i1 + 1],
				a_settings
			);
		}
	}
...
function f_offset_point(a_1, a_2, a_settings) { //左手系に注意
	...
	const c_segment_pair_key = "segment_pair_key_" + String(c_s1["sx"])
		+ "_" + String(c_s1["sy"])
		+ "_" + String(c_s1["ex"])
		+ "_" + String(c_s1["ey"])
		+ "_" + String(c_s2["sx"])
	...
	if (c_z["parallel"] === true || (c_z["parallel_degree"] < 0.05 && c_s1z !== c_s2z)) { //3点
		if (Math.abs(c_s1z) <= Math.abs(c_s2z)) {
			c_xy.push({
				"curve": false,
				"x": c_s2z * c_z["xy"][2]["x"][0] + c_s2z * c_z["xy"][2]["x"][1] + c_z["xy"][2]["x"][2],
				"y": c_s2z * c_z["xy"][2]["y"][0] + c_s2z * c_z["xy"][2]["y"][1] + c_z["xy"][2]["y"][2]
			});
		} else {
			c_xy.push({
				"curve": false,
				"x": c_s1z * c_z["xy"][0]["x"][0] + c_s1z * c_z["xy"][0]["x"][1] + c_z["xy"][0]["x"][2],
				"y": c_s1z * c_z["xy"][0]["y"][0] + c_s1z * c_z["xy"][0]["y"][1] + c_z["xy"][0]["y"][2]
			});
		}
		c_s1["et"] = 1;
		c_s2["st"] = 0;
		...
		c_z["parallel"] === false
		&& a_settings["curve"] === true
		&& c_s1["et"] > 1
		&& c_s2["st"] < 0) { //3点、曲線
		c_xy.push({
			"curve": false,
			"x": c_s1z * c_z["xy"][0]["x"][0] + c_s2z * c_z["xy"][0]["x"][1] + c_z["xy"][0]["x"][2],
			"y": c_s1z * c_z["xy"][0]["y"][0] + c_s2z * c_z["xy"][0]["y"][1] + c_z["xy"][0]["y"][2]
		});
		c_xy.push({
			"curve": true,
			"x": c_s1z * c_z["xy"][1]["x"][0] + c_s2z * c_z["xy"][1]["x"][1] + c_z["xy"][1]["x"][2],
			"y": c_s1z * c_z["xy"][1]["y"][0] + c_s2z * c_z["xy"][1]["y"][1] + c_z["xy"][1]["y"][2]
		});
		c_xy.push({
			"curve": false,
			"x": c_s1z * c_z["xy"][2]["x"][0] + c_s2z * c_z["xy"][2]["x"][1] + c_z["xy"][2]["x"][2],
			"y": c_s1z * c_z["xy"][2]["y"][0] + c_s2z * c_z["xy"][2]["y"][1] + c_z["xy"][2]["y"][2]
		});
	} else { //中の点のみ
		c_xy.push({
			"curve": false,
...
function f_offset(a_1, a_2) { //左手系に注意
	...
	if (
		(c_p1x === c_p2x && c_p1y === c_p2y)
		|| (c_p3x === c_p4x && c_p3y === c_p4y)
	) {
		console.log("点あり");
	}
	...
	//平行を確認する
	if (
		(c_p2x - c_p1x) * (c_p4y - c_p3y) === (c_p2y - c_p1y) * (c_p4x - c_p3x)
	) {
		console.log("平行");
		l_parallel = true;
	}
	...
	return {
		"parallel_degree": Math.abs(c_xyxynn),
		"parallel": l_parallel_tf,
		"d1t": [
			(-1) * c_yxyx * c_xxyy / c_v1n, c_yxyx * c_v2n,
			1 + l_d1t
		],
		"d2t": [
			(-1) * c_yxyx * c_v1n, c_yxyx * c_xxyy / c_v2n,
			l_d2t
		],
		"xy": [
			{
				...
				"x": [
					(-1) * c_v1n * c_v2x * c_yxyx,
					c_v2n * c_v1x * c_yxyx,
					c_pcx
				],
				"y": [
					(-1) * c_v1n * c_v2y * c_yxyx,
					c_v2n * c_v1y * c_yxyx,
					c_pcy
				]
			}, {
...
function f_cross_point(a_x1, a_y1, a_x2, a_y2, a_x3, a_y3, a_x4, a_y4) {
	...
	if (c_3 === 0) { //平行によりうまく求められないとき。
		...
		return {
			"x": (c_1 * c_vx2 - c_2 * c_vx1) / c_3,
			"y": (c_vy1 * c_2 - c_vy2 * c_1) / c_3,
			"parallel": false
		};
	}
}