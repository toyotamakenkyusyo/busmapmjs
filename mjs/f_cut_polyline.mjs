export function f_cut_polyline(a_polyline, a_stop_array) {
	...
	*/
	...
	for (let i1 = 0; i1 < c_cut_stop_number_array.length - 1; i1++) {
		...
		c_polylines[c_id] = [];
		for (
			let i2 = c_cut_stop_number_array[i1]["number"];
			i2 <= c_cut_stop_number_array[i1 + 1]["number"];
			i2++
		) {
			if (i2 === c_cut_stop_number_array[i1]["number"]) { //最初
				c_polylines[c_id].push({
					"polyline": [],
					"width": a_polyline[i2 + 1]["width"]
				});
			} else if (
				i2 < c_cut_stop_number_array[i1 + 1]["number"]
				&& a_polyline[i2]["width"] !== a_polyline[i2 + 1]["width"]
			) { //太さが変わるとき
				c_polylines[c_id][c_polylines[c_id].length - 1]["polyline"].push({
					"lon": a_polyline[i2]["lon"],
					"lat": a_polyline[i2]["lat"]
				});
				c_polylines[c_id].push({
					"polyline": [],
					"width": a_polyline[i2 + 1]["width"]
	...
	for (let i1 = 0; i1 < c_cut_stop_number_array.length - 1; i1++) {
		...
		for (
			...
			if (i2 === c_cut_stop_number_array[i1]["number"]) { //最初
				...
				c_curves[c_id][c_curves[c_id].length - 1]["curve"].push([
					a_polyline[i2]["lat"],
					a_polyline[i2]["lon"]
				]);
				//SVG
				c_svg_paths[c_id].push({
					"d": "",
					"stroke-width": a_polyline[i2 + 1]["width"]
				});
				c_svg_paths[c_id][c_svg_paths[c_id].length - 1]["d"] +=
					"M"
					+ a_polyline[i2]["x"]
					+ ","
					+ a_polyline[i2]["y"];
				...
				//前の点
				if (
					a_polyline[i2 - 1]["curve"] === true
					&& a_polyline[i2 - 1]["width"] === a_polyline[i2]["width"]
				) { //前が曲線で、太さが変わっていない
					c_curves[c_id][c_curves[c_id].length - 1]["curve"].push([
						a_polyline[i2]["lat"], a_polyline[i2]["lon"]
					]);
					//SVG
					c_svg_paths[c_id][c_svg_paths[c_id].length - 1]["d"] +=
						","
	...
	for (let i1 = 0; i1 < c_cut_stop_number_array.length; i1++) {
		c_stop_array.push({
			"lon": a_polyline[c_cut_stop_number_array[i1]["number"]]["lon"],
			"lat": a_polyline[c_cut_stop_number_array[i1]["number"]]["lat"]
		});
		c_stop_circles.push({
			"x": a_polyline[c_cut_stop_number_array[i1]["number"]]["x"],
			"y": a_polyline[c_cut_stop_number_array[i1]["number"]]["y"]
		});
	}

	return {
		"polylines": c_polylines,
		"curves": c_curves,
		"stop_array": c_stop_array,
		"stop_circles": c_stop_circles,
		"svg_paths": c_svg_paths
	};
}